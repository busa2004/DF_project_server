...
public class TaskService {
	...
	public List<TaskResponse> getAllTaskNoPage() {
		...
		for (int i = list.size() - 1; i > -1; i--) {
			responseList
					.add(new TaskResponse(list.get(i).getId(), list.get(i).getTitle(), list.get(i).getContent(), i));
		}
		return responseList;
	}
	...
	public List<TaskResponse> getTaskSelect(TaskRequest taskRequest) {
		...
		for (int i = list.size() - 1; i > -1; i--) {
			...
			responseList.add(
					new TaskResponse(list.get(i).getId(), list.get(i).getTitle(), list.get(i).getContent(), i, status));
	...
	public List<TaskResponse> getAllTask(TaskRequest taskRequest) {
		List<TaskResponse> list = taskRepository.findAllByStatus(Status.valueOf(taskRequest.getStatus()),
				taskRequest.getSearch(), taskRequest.getFrom(), taskRequest.getTo());
